...
class IndexAgentNenya(
    ...
    captureType: DefaultCaptureType = DefaultCaptureType.Note,
    ...
    companion object {
        fun agentContext(captureType: DefaultCaptureType): String {
            val capture = captureType.promptNoun
            return """
...
## Interpretation guidelines:
 - Create a $capture with the user's input unless they specify a different action, do not assume an action that wasn't explicitly requested, just make a $capture.
 - Avoid asking follow-up questions unless necessary.
 - When user requests are ambiguous, always lean towards creating a $capture; for example if the user doesn't ask for a timer don't create a timer, even if the request has a duration in it.
...
both as the content unless it's clearly two separate actions, for example 'remind me to buy milk tomorrow and bread the day after' should create two reminders.
 - When fulfilling a user request, do not also passively take a $capture if you have already taken another requested action.
...
## Response and action guidelines:
 ...
 - Always take an action, even if you just fall back to creating a $capture with what the user said. The only exception is a request for a timer, alarm, or scheduled reminder where the time was cut off or never heard, as described above.
